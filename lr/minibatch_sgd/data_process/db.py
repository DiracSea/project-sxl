...
class conn_rand(conn_db):#yield row
    def __init__(self,db,table,host,port,user,psw):
        super(conn_rand,self).__init__(db,table,host,port,user,psw)
        
    def export(self):
        super(conn_rand,self).export()
#2
class conn_all(conn_db):
    def __init__(self,db,table,host,port,user,psw):
        super(conn_all,self).__init__(db,table,host,port,user,psw)
    ...
    def export(self):
        try:
            ...
            while 1:
                row = cur.fetchone()#use fetchone but not fetchall
                if row:
                    table.append(list(row)) #generate a generator
                else:
                    break
            return table
